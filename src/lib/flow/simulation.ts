...
export function computeDirectHits(
  values: Record<ConstantId, number>
  ...
  for (const node of chainNodes) {
    ...
    for (const s of sens) {
      if (s.role === "critical" && isOutOfRange(s.constantId, values[s.constantId])) {
        outcome = { status: "fails", reason: s.effect, dueToConstant: s.constantId };
        break;
      }
    }
    if (outcome.status !== "fails") {
      for (const s of sens) {
        ...
        if ((s.role === "contributing" && out) || (off && !out)) {
          outcome = { status: "altered", reason: s.effect, dueToConstant: s.constantId };
          break;
        }
      }
    }
...
// --- Fase 3: ringkasan ---
export function summarize(outcomes: Map<string, NodeOutcome>): {
  firstFailure: SimulationResult["firstFailure"];
  failedInOrder: string[];
  counts: SimulationResult["counts"];
} {
  const failed = chainNodes.filter((n) => outcomes.get(n.id)?.status === "fails");
  const failedInOrder = [...failed].sort((a, b) => a.timeValue - b.timeValue).map((n) => n.id);
  ...
  return { firstFailure, failedInOrder, counts };
}
...
export function simulate(values: Record<ConstantId, number>): SimulationResult {
  ...
  const outcomes = propagateCascade(computeDirectHits(values));
